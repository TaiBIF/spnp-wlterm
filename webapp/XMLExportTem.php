...
ini_set("memory_limit","24M");
  ...
  $sqlstr="SELECT count(0) FROM (temperature INNER JOIN project ON temperature.Project_id = project.Project_id) INNER JOIN station ON temperature.id = station.id";
  $result=mysql_query($sqlstr,$link_ID);
  $datas=mysql_fetch_row($result); 
  $sn_index=$datas[0];
  $groups=intval(($sn_index-1)/1000)+1;
  for ($group=0; $group < $groups;$group++){
    $sqlstr="SELECT temperature.*, station.*, project.* FROM (temperature INNER JOIN project ON temperature.Project_id = project.Project_id) INNER JOIN station ON temperature.id = station.id  LIMIT ". $group*1000 . ",1000";
    $result=mysql_query($sqlstr,$link_ID);
    $sn_index=mysql_num_rows($result);  
    for ($index=0; $index < $sn_index ; $index++){
     $arr[$index]=mysql_fetch_array($result); 
     $filename="d:/wlterm/xml2/WLTERMTemp" . $arr[$index]['record_id'] . ".xml";
     $fp = fopen($filename, 'w');
...
fwrite($fp, "<dwc:occurrenceID>urn:lsid:wlterm.biodiv.sinica.edu.tw:observation:TEMP" . $arr[$index]['record_id']."</dwc:occurrenceID>");
fwrite($fp, "<dwc:catalogNumber>".$arr[$index]['record_id']."</dwc:catalogNumber>");
		 ...
		 fwrite($fp,"<AirTemperature Unit='C'>".$arr[$index]['air'] ."</AirTemperature>"); 
		 fwrite($fp,"<WaterTemperature Unit='C'>".$arr[$index]['water'] ."</WaterTemperature>"); 
		 fwrite($fp,"<SoilSurfaceTemperature Unit='C'>".$arr[$index]['soil_0cm'] ."</SoilSurfaceTemperature>"); 
		 fwrite($fp,"<SoilTemperatureUnder25cm Unit='C'>".$arr[$index]['soil_25cm'] ."</SoilTemperatureUnder25cm>"); 
		 fwrite($fp,"<SoilTemperatureUnder50cm Unit='C'>".$arr[$index]['soil_50cm'] ."</SoilTemperatureUnder50cm>"); 
		 fwrite($fp,"<SoilTemperatureUnder65cm Unit='C'>".$arr[$index]['soil_65cm'] ."</SoilTemperatureUnder65cm>"); 
		 fwrite($fp,"<SoilTemperatureUnder90cm Unit='C'>".$arr[$index]['soil_90cm'] ."</SoilTemperatureUnder90cm>"); 
