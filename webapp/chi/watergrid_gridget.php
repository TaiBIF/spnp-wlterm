...
include("../conn_clean.php");
   ...
   if ($_GET["cr_mask"]=="equal"){
    ...
    if (!isset($_GET["direction"]) || $_GET["direction"]=="asc" || $_GET["direction"]=="undefined")
      ...
      $_GET["direction"]="DESC";
   $sqlstr="SELECT water.record_id,station.id as sid,station.locality_chinese,station.latitude,station.longitude,station.maximum_elevation,station.maximum_depth,water.date FROM water INNER JOIN station ON water.id = station.id where  (project_id=3 or project_id=4) limit 0,1";  
...
function getDataFromDB($name_mask,$code_mask,$sort_by,$sort_dir,$qb,$qe){
		...
		$sql="SELECT water.record_id ,station.id as sid,station.locality_chinese,station.latitude,station.longitude,station.maximum_elevation,station.maximum_depth,water.date FROM water INNER JOIN station ON water.id = station.id where  (project_id=3 or project_id=4) ";
		if($sc_mask!='' && $fa_mask!='')
		...
		if($res){
			while($row=mysql_fetch_array($res,MYSQL_NUM)){
				   ...
				   for ($i=1 ; $i < $num; $i++){
				    if ($i==7){
				     print("<cell><![CDATA[&nbsp;&nbsp;".substr($row[$i],0,10)."]]></cell>");
					}else{
				     print("<cell><![CDATA[&nbsp;&nbsp;".$row[$i]."]]></cell>");
					 
					} 
				   };
	               print("<cell><![CDATA[<a href='WaterQ2.php?record_id=". $row[0]."&PI=3'><img border='0' src='../images/smallearth.gif' height='10'></a>]]></cell>");	
				print("</row>");
				$posStart++;
			}
		}else{
