...
<script>
	    ...
	    var flds ="界中文,界名,門中文,門名,綱中文,綱名,目中文,目名,<center>數量</center>,查詢,地圖";
		mygrid.setHeader(flds);
		mygrid.setInitWidths("72,72,80,80,80,80,90,120,65,45,45")
		mygrid.setColAlign("left,left,left,left,left,left,left,left,center,center,center");
		mygrid.setColTypes("ed,ed,ed,ed,ed,ed,ed,ed,ro,ro,ro");
		mygrid.setColumnIds(flds);
		mygrid.setSkin("light");
		mygrid.setColSorting("str,str,str,str,str,str,str,str,int,str,str");
			...
			//mygrid.setOnLoadingStart(showLoading)
			       				//mygrid.splitAt(1);
            if (queryPair[0]!="D1"){
		    mygrid.loadXML("ordergrid_gridget.php?un="+Date.parse(new Date()));
			}else{
		    mygrid.loadXML("ordergrid_gridget.php?sc_mask=id&cr_mask=equal&fa_mask="+valuePair[0]);
			}
			showLoading();
	//============================================================================================
...
//============================================================================================
		...
		function setCounter(){
			...
			span.style.color = "";
			span.innerHTML = "<font class='mydefine5'>共 "+mygrid.getRowsNum()+" 目</font>";
		}
		...
		function reloadGrid(){
			...
			mygrid.clearAll();
			mygrid.loadXML("ordergrid_gridget.php?sc_mask="+escape(sc_mask)+"&fa_mask="+escape(fa_mask)+"&cr_mask="+escape(cr_mask)+"&orderBy="+window.s_col+"&direction="+window.a_direction);
...
<table cellspacing="0" cellpadding="0" border="0">
	<tr valign="middle">
		...
		<td width="180" align="left" valign="middle"><div class="mydefine3">調查生物目別數量統計</div></td>
...
document.getElementById("pfield").value = "order";
