...
PassValue();
	...
	<script>
	   ...
	   // var flds ="family,scientific_name,date,id,locality_chinese,latitude,longitude,collector_chinese,examine_way,identified_by_chinese,xml";
	    var flds ="測站,門名,綱名,目名,科名,學名,中文名,調查日,地點,緯度,經度,調查者,調查法,鑑定者,內容";
		mygrid.setHeader(flds);
		mygrid.setInitWidths("60,40,40,40,100,155,75,70,70,45,45,50,50,50,38")
		mygrid.setColAlign("center,left,left,left,left,left,left,left,left,left,left,left,left,left,center");
		mygrid.setColTypes("ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ro");
		mygrid.setColumnIds(flds);
		mygrid.setSkin("light");
		mygrid.setColSorting("int,str,str,str,str,str,str,date,str,int,int,str,str,str,str");
		    ...
		    mygrid.loadXML("recordgrid_gridget.php?un="+Date.parse(new Date()));
			}else{
		    mygrid.loadXML("recordgrid_gridget.php?sc_mask="+queryPair[0]+"&cr_mask=equal&fa_mask="+valuePair[0]);
			}			
			showLoading();
	//============================================================================================
	//myDataProcessor = new dataProcessor("root_gridupdate.php");
...
//============================================================================================
		...
		function setCounter(){
			...
			span.style.color = "";
			span.innerHTML = "<font class='mydefine5'>共 "+mygrid.getRowsNum()+" 筆</font>";
		}
		...
		function reloadGrid(){
			...
			mygrid.clearAll();
			mygrid.loadXML("recordgrid_gridget.php?sc_mask="+escape(sc_mask)+"&fa_mask="+escape(fa_mask)+"&cr_mask="+escape(cr_mask)+"&orderBy="+window.s_col+"&direction="+window.a_direction);
...
<table cellspacing="0" cellpadding="0" border="0">
	<tr valign="middle">
		...
		<td width="140" align="left" valign="middle"><div class="mydefine3">生物物種調查紀錄</div></td>
		<td width="10" align="center"></td>
		<td width="350" align="left" valign="middle">
        <select class="mydefine" id="pfield" >
        <option class="mydefine" value="sid">測站</option>
        ...
        <option class="mydefine" value="date">調查日</option>
        <option class="mydefine" value="locality_chinese">地點</option>
        <option class="mydefine" value="collector_chinese">調查者</option>
        <option class="mydefine" value="examine_waye">調查方法</option>
        <option class="mydefine" value="identified_by_chines">鑑定者</option>
        </select>
	    ...
	    <input class="mydefine7" type="text" id="pvalue"><button class="mydefine7" onClick="reloadGrid()" id="submitButton" style="margin-left:3px;">搜尋</button></td>
...
function locamap(){
var plink="locamap.php?pfield="+document.getElementById("pfield").value+"&pcr="+document.getElementById("pcr").value+"&pvalue="+escape(document.getElementById("pvalue").value);
window.location.href = plink;
}
</script>
<a><img onclick='locamap()' border="0" src="../images/googlemap2.gif" align="absmiddle" height="20"></a>
        </td>
        <td></td>
        <td>
