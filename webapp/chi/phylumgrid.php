...
<script>
	    ...
	    var flds ="&nbsp;&nbsp;界中文,&nbsp;&nbsp;界名,&nbsp;&nbsp;門中文,&nbsp;&nbsp;門名,<center>數量</center>,查詢,地圖";
		mygrid.setHeader(flds);
		mygrid.setInitWidths("140,140,150,147,80,45,45")
		mygrid.setColAlign("left,left,left,left,center,center,center");
		mygrid.setColTypes("ed,ed,ed,ed,ed,ro,ro");
		...
		mygrid.setColSorting("str,str,str,str,int,str,str");
			...
			//mygrid.setOnLoadingStart(showLoading)
			       				//mygrid.splitAt(1);
            if (queryPair[0]!="D1"){
		    mygrid.loadXML("phylumgrid_gridget.php?un="+Date.parse(new Date()));
			}else{
		    mygrid.loadXML("phylumgrid_gridget.php?sc_mask=id&cr_mask=equal&fa_mask="+valuePair[0]);
			}
			showLoading();
	//============================================================================================
...
//============================================================================================
		...
		function setCounter(){
			...
			span.style.color = "";
			span.innerHTML = "<font class='mydefine5'>共 "+mygrid.getRowsNum()+" 門</font>";
		}
		...
		function reloadGrid(){
			...
			mygrid.clearAll();
			mygrid.loadXML("phylumgrid_gridget.php?sc_mask="+escape(sc_mask)+"&fa_mask="+escape(fa_mask)+"&cr_mask="+escape(cr_mask)+"&orderBy="+window.s_col+"&direction="+window.a_direction);
...
<table border="0" cellpadding="0" cellspacing="0" width="75%" height="99%"><tr><td valign="top"> 

<table cellspacing="0" cellpadding="0" border="0">
	<tr valign="middle">
		<td width="105" align="center" valign="middle"></td>
		<td width="175" align="left" valign="middle"><div class="mydefine3">調查生物門別數量統計</div></td>
...
document.getElementById("pfield").value = "phylum";
