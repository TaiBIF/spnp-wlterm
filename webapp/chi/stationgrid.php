...
<script>
	    ...
	    var flds ="測站,緯度,經度,誤差,地名,最高海拔,最低海拔,地點描述,x,y,備註";
		mygrid.setHeader(flds);
		mygrid.setInitWidths("45,70,70,45,190,68,68,180,70,70,50")
		mygrid.setColAlign("center,left,left,center,left,left,left,left,center,center,left");
		mygrid.setColTypes("ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ed");
		mygrid.setColumnIds(flds);
		mygrid.setSkin("light");
		mygrid.setColSorting("int,int,int,int,str,str,str,str,int,int,str");
			...
			if (mygrid.setOnLoadingEnd)
				...
				mygrid.sortField=function(){
					mygrid.setColSorting("int,str,date,int,int,int,int,int,int,int,str,str");
		    ...
		    mygrid.loadXML("stationgrid_gridget.php?un="+Date.parse(new Date()));
			}else{
		    mygrid.loadXML("stationgrid_gridget.php?sc_mask=id&cr_mask=equal&fa_mask="+valuePair[0]);
			}
			showLoading();
	//============================================================================================
...
//============================================================================================
		...
		function reloadGrid(){
			...
			mygrid.clearAll();
			mygrid.loadXML("stationgrid_gridget.php?sc_mask="+escape(sc_mask)+"&fa_mask="+escape(fa_mask)+"&cr_mask="+escape(cr_mask)+"&orderBy="+window.s_col+"&direction="+window.a_direction);
...
<table cellspacing="0" cellpadding="0" border="0">
	<tr valign="middle">
		...
		<td width="100" align="left" valign="middle"><div class="mydefine3">測站資料</div></td>
		<td width="10" align="center"></td>
		<td width="330" align="left" valign="middle">
        <select class="mydefine" id="pfield" >
        <option class="mydefine" value="id">測站</option>
        <option class="mydefine" value="locality_chinese">地名</option>
        </select>
...
document.getElementById("pfield").value = "id";
