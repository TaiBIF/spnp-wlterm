...
<script>
	    ...
	    var flds ="河川,日期,公告流量,模擬流量,模擬上界,模擬下界,思源雨量,桃山雨量,環山雨量,松茂雨量";
		mygrid.setHeader(flds);
		mygrid.setInitWidths("90,90,80,80,80,80,80,80,80,80")
		mygrid.setColAlign("center,left,center,center,center,center,center,center,center,center");
		mygrid.setColTypes("ed,ed,ed,ed,ed,ed,ed,ed,ro,ro");
		mygrid.setColumnIds(flds);
		mygrid.setSkin("light");
		mygrid.setColSorting("str,date,int,int,int,int,int,int,int,int");
			...
			//mygrid.setOnLoadingStart(showLoading)
			       				//mygrid.splitAt(1);
            if (queryPair[0]!="D1"){
		    mygrid.loadXML("flowgrid_gridget.php?un="+Date.parse(new Date()));
			}else{
		    mygrid.loadXML("flowgrid_gridget.php?sc_mask=id&cr_mask=equal&fa_mask="+valuePair[0]);
			}
			showLoading();
	//============================================================================================
...
//============================================================================================
		...
		function reloadGrid(){
			...
			mygrid.clearAll();
			mygrid.loadXML("flowgrid_gridget.php?sc_mask="+escape(sc_mask)+"&fa_mask="+escape(fa_mask)+"&cr_mask="+escape(cr_mask)+"&orderBy="+window.s_col+"&direction="+window.a_direction);
		...
		alert("請先選取一列，再按新增");
		 ...
		 function gcheckbox(pobj){
			if (pobj.checked==true){
				    var flds2 = "-,-,cms,cms,cms,cms,mm,mm,mm,mm";
...
<table border="0" cellpadding="0" cellspacing="0" width="85%" height="99%"><tr><td valign="top"> 

<table cellspacing="0" cellpadding="0" border="0">
	<tr valign="middle">
		<td width="150" align="center" valign="middle"></td>
		<td width="110" align="left" valign="middle"><div class="mydefine3">河川流量推估</div></td>
		<td width="10" align="center"></td>
		<td width="350" align="left" valign="middle">
        <select class="mydefine" id="pfield" >
        <option class="mydefine" value="station.id">河川</option>
        <option class="mydefine" value="date">調查日期</option>
        </select>
	...
	<td><input onClick="gcheckbox(this)" type="checkbox" name="checkbox1"></td><td align="left"  valign="middle" nowrap><font class="mydefine2">單位</font></td><td align="left"  valign="middle" nowrap>
