...
include("../conn_clean.php");
   ...
   if ($_GET["cr_mask"]=="equal"){
    ...
    if (!isset($_GET["direction"]) || $_GET["direction"]=="asc" || $_GET["direction"]=="undefined")
      ...
      $_GET["direction"]="DESC";
 $sqlstr="SELECT id,phylum_c,phylum,class_c,`class`,order_c,`order`,family_c,family,scientific_name,chinese,author,view_wlspecies.total from view_wlspecies limit 0,1";    
...
function getDataFromDB($name_mask,$code_mask,$sort_by,$sort_dir,$qb,$qe){
		...
		$sql="SELECT id,phylum_c,phylum,class_c,`class`,order_c,`order`,family_c,family,scientific_name,chinese,author,total from view_wlspecies Where 1=1";
		if($sc_mask!='' && $fa_mask!='')
		...
		if($res){
			while($row=mysql_fetch_array($res,MYSQL_NUM)){
	               ...
	               print("<cell><![CDATA[<a href='recordgrid.php?scientific_name=". $row[9]."'><img border='0' src='../images/smallearth.gif' height='10'></a>]]></cell>");				
	               print("<cell><![CDATA[<a href='locamap.php?pfield=scientific_name&pcr=equal&pvalue=". $row[9]."&ch=".phpescape($row[10])."'><img border='0' src='../images/smallearth.gif' height='10'></a>]]></cell>");				
				print("</row>");
				$posStart++;
			}
		}else{
