...
<script>
		...
		mygrid.setColSorting("int,str,str,str,str,int,str,str");
			...
			//mygrid.setOnLoadingStart(showLoading)
			       				//mygrid.splitAt(1);
            if (queryPair[0]!="D1"){
		    mygrid.loadXML("alagegrid_gridget.php?un="+Date.parse(new Date()));
			}else{
		    mygrid.loadXML("alagegrid_gridget.php?sc_mask=id&cr_mask=equal&fa_mask="+valuePair[0]);
			}
			showLoading();
	//============================================================================================
...
//============================================================================================
		...
		function reloadGrid(){
			...
			mygrid.clearAll();
			mygrid.loadXML("alagegrid_gridget.php?sc_mask="+escape(sc_mask)+"&fa_mask="+escape(fa_mask)+"&cr_mask="+escape(cr_mask)+"&orderBy="+window.s_col+"&direction="+window.a_direction);
...
<table cellspacing="0" cellpadding="0" border="0">
	<tr valign="middle">
		...
		<td width="180" align="left" valign="middle"><div class="mydefine3">藻類與有機碎屑</div></td>
