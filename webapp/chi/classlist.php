...
if ($key=="請輸入關鍵字"){$key="";}
if ($key==""){
 $sqlstr="SELECT count(0) from view_wlclass order by " . $pporder;
}else{
 $sqlstr="SELECT count(0) from view_wlclass where kingdom_c like '%" . $key . "%' or kingdom like '%" . $key . "%' or phylum_c like '%" . $key . "%' or phylum like '%" . $key . "%' or class_c like '%" . $key . "%' or `class` like '%" . $key . "%' order by " . $pporder;
}
...
<form name="form1" action="ClassList.php">
  <center>
  <?php if ($key==""){?>
  <font size="3" color="#393939"><B>全部測站共調查&nbsp;<?php echo $sn_index?> 綱生物</B></font>&nbsp;&nbsp;&nbsp;
  <?php }else{?>
...
<table border="1" height="49" cellspacing="1" cellpadding="2">         
   <tr>         
    ...
    <td class=tdHead nowrap>綱名<a href="classlist.php?page=<?php echo $page?>&R1=`class`&D1=<?php echo $pcrit?>&pz=<?php echo $pz?>&search=<?php echo $key?>"><img border="0" src="../images/arrow.gif" align="absmiddle" hspace="2" vspace="2" title="排序" <?php echo $p5?>></a></font></td>         
    <td class=tdHead nowrap> 綱中文名 <a href="classlist.php?page=<?php echo $page?>&R1=class_c&D1=<?php echo $pcrit?>&pz=<?php echo $pz?>&search=<?php echo $key?>"><img border="0" src="../images/arrow.gif" align="absmiddle" hspace="2" vspace="2" title="排序" <?php echo $p6?>></a></font> </td>         
...
<?php
if ($key==""){
 $sqlstr="SELECT * from view_wlclass order by " . $pporder. " LIMIT ". ($page-1)*$pz . ", ". $pz;
}else{
 $sqlstr="SELECT * from view_wlclass where kingdom_c like '%" . $key . "%' or kingdom like '%" . $key . "%' or phylum_c like '%" . $key . "%' or phylum like '%" . $key . "%' or class_c like '%" . $key . "%' or `class` like '%" . $key . "%' order by " . $pporder. " LIMIT ". ($page-1)*$pz . ", ". $pz;
}
 $result=mysql_query($sqlstr,$link_ID);
  if ($result !="") {
  ...
  for ($index=0; $index < $sn2_index ; $index++){
   ...
   $arr[$index]=mysql_fetch_array($result);                
     ...
     echo "<td align='left' class=".$tdclass."><a href='specieslist2.php?cl=". $arr[$index]['class']."&ch=".urlencode($arr[$index]['class_c'])."'>". $arr[$index]['class']."</a>&nbsp;&nbsp;</td>";  
     echo "<td align='left' class=".$tdclass.">". $arr[$index]['class_c']."</td>";
     echo "<td  class=".$tdclass." align='center'><a href='specieslist2.php?cl=". $arr[$index]['class']."&ch=".urlencode($arr[$index]['class_c'])."'><center>". $arr[$index]['total']."</a></td></tr>";
    }; 
	}      
    ?>        
  </table>       
