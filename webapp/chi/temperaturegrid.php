...
<script>
	    ...
	    var flds ="測站,測站站名,調查日期,氣溫,水溫,土表溫,土下25cm,土下50cm,土下65cm,土下90cm,xml";
		mygrid.setHeader(flds);
		mygrid.setInitWidths("60,140,140,60,60,60,60,60,60,60,55")
		mygrid.setColAlign("center,left,left,left,center,center,center,center,center,,center,center");
		mygrid.setColTypes("ed,ed,ed,ed,ed,ed,ed,ed,ed,ed,ro");
		mygrid.setColumnIds(flds);
		mygrid.setSkin("light");
		mygrid.setColSorting("int,str,date,int,int,int,int,int,int,int,str");
			...
			//mygrid.setOnLoadingStart(showLoading)
			       				//mygrid.splitAt(1);
            if (queryPair[0]!="D1"){
		    mygrid.loadXML("temperaturegrid_gridget.php?un="+Date.parse(new Date()));
			}else{
		    mygrid.loadXML("temperaturegrid_gridget.php?sc_mask=id&cr_mask=equal&fa_mask="+valuePair[0]);
			}
			showLoading();
	//============================================================================================
...
//============================================================================================
		...
		function reloadGrid(){
			...
			mygrid.clearAll();
			mygrid.loadXML("temperaturegrid_gridget.php?sc_mask="+escape(sc_mask)+"&fa_mask="+escape(fa_mask)+"&cr_mask="+escape(cr_mask)+"&orderBy="+window.s_col+"&direction="+window.a_direction);
...
<table cellspacing="0" cellpadding="0" border="0">
	<tr valign="middle">
		...
		<td width="170" align="left" valign="middle"><div class="mydefine3">環境日均溫監測資料</div></td>
		<td width="10" align="center"></td>
		<td width="350" align="left" valign="middle">
        <select class="mydefine" id="pfield" >
        <option class="mydefine" value="station.id">測站</option>
        <option class="mydefine" value="locality_chinese">測站站名</option>
