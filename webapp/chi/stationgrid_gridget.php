...
include("../conn_clean.php"); 
 ...
 $sqlstr="SELECT auto_id,id,latitude,longitude,coordinate_precision,locality_chinese,minimum_elevation,maximum_elevation,locality_describe,x,y,note FROM station limit 0,1";    
...
function getDataFromDB($name_mask,$code_mask,$sort_by,$sort_dir,$qb,$qe){
		...
		$sql = "SELECT auto_id,id,latitude,longitude,coordinate_precision,locality_chinese,minimum_elevation,maximum_elevation,locality_describe,x,y,note FROM station Where 1=1";
		if($sc_mask!='' && $fa_mask!='')
		...
		if($res){
			while($row=mysql_fetch_array($res,MYSQL_NUM)){
				   ...
				   for ($i=1 ; $i < $num; $i++){
				    if ( $i==1){ 
				     print("<cell><![CDATA[<a href='locamap.php?id=". $row[$i]."'>".$row[$i]."</a>]]></cell>");
					}else{
