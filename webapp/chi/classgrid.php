...
<script>
	    ...
	    var flds ="界中文,界名,門中文,門名,綱中文,綱名,<center>數量</center>,查詢,地圖";
		mygrid.setHeader(flds);
		mygrid.setInitWidths("90,90,110,110,110,110,85,45,45")
		mygrid.setColAlign("left,left,left,left,left,left,center,center,center");
		mygrid.setColTypes("ed,ed,ed,ed,ed,ed,ro,ro,ro");
		...
		mygrid.setColSorting("str,str,str,str,str,str,int,str,str");
			...
			//mygrid.setOnLoadingStart(showLoading)
			       				//mygrid.splitAt(1);
            if (queryPair[0]!="D1"){
		    mygrid.loadXML("classgrid_gridget.php?un="+Date.parse(new Date()));
			}else{
		    mygrid.loadXML("classgrid_gridget.php?sc_mask=id&cr_mask=equal&fa_mask="+valuePair[0]);
			}
			showLoading();
	//============================================================================================
...
//============================================================================================
		...
		function setCounter(){
			...
			span.style.color = "";
			span.innerHTML = "<font class='mydefine5'>共 "+mygrid.getRowsNum()+" 綱</font>";
		}
		...
		function reloadGrid(){
			...
			mygrid.clearAll();
			mygrid.loadXML("classgrid_gridget.php?sc_mask="+escape(sc_mask)+"&fa_mask="+escape(fa_mask)+"&cr_mask="+escape(cr_mask)+"&orderBy="+window.s_col+"&direction="+window.a_direction);
...
<table border="0" cellpadding="0" cellspacing="0" width="80%" height="99%"><tr><td valign="top"> 

<table cellspacing="0" cellpadding="0" border="0">
	<tr valign="middle">
		<td width="115" align="center" valign="middle"></td>
		<td width="175" align="left" valign="middle"><div class="mydefine3">調查生物綱別數量統計</div></td>
...
document.getElementById("pfield").value = "class";
