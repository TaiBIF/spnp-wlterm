...
include("../conn_clean.php");
   ...
   if ($_GET["cr_mask"]=="equal"){
    ...
    if (!isset($_GET["direction"]) || $_GET["direction"]=="asc" || $_GET["direction"]=="undefined")
      ...
      $_GET["direction"]="DESC";
   $sqlstr="SELECT flow.record_id,flow_stations.name,flow.`date`,flow.public,flow.simu,flow.`max`,flow.`min`,rain.st1,rain.st2,rain.st3,rain.st4 FROM flow Inner Join flow_stations ON flow.station_id = flow_stations.id Inner Join rain ON flow.`date` = rain.`date` limit 0,1";  
...
function getDataFromDB($name_mask,$code_mask,$sort_by,$sort_dir,$qb,$qe){
		...
		$sql="SELECT flow.record_id,flow_stations.name,flow.`date`,flow.public,flow.simu,flow.`max`,flow.`min`,rain.st1,rain.st2,rain.st3,rain.st4 FROM flow Inner Join flow_stations ON flow.station_id = flow_stations.id Inner Join rain ON flow.`date` = rain.`date`where  (1=1) ";
		if($sc_mask!='' && $fa_mask!='')
