...
include("../conn_clean.php");
   ...
   if ($_GET["cr_mask"]=="equal"){
    ...
    if (!isset($_GET["direction"]) || $_GET["direction"]=="asc" || $_GET["direction"]=="undefined")
      ...
      $_GET["direction"]="DESC";
   $sqlstr="SELECT temperature.record_id,temperature.id as sid, station.locality_chinese,temperature.date,temperature.air,temperature.water,temperature.soil_0cm,temperature.soil_25cm,temperature.soil_50cm,temperature.soil_65cm,temperature.soil_90cm FROM temperature INNER JOIN station ON temperature.id = station.id limit 0,1";  
...
function getDataFromDB($name_mask,$code_mask,$sort_by,$sort_dir,$qb,$qe){
		...
		$sql="SELECT temperature.record_id,temperature.id as sid, station.locality_chinese,temperature.date,temperature.air,temperature.water,temperature.soil_0cm,temperature.soil_25cm,temperature.soil_50cm,temperature.soil_65cm,temperature.soil_90cm FROM temperature INNER JOIN station ON temperature.id = station.id where 1=1  ";
		if($sc_mask!='' && $fa_mask!='')
		...
		if($res){
			while($row=mysql_fetch_array($res,MYSQL_NUM)){
	               ...
	               print("<cell><![CDATA[<a href='../xml_tem.php?record_id=". $row[0]."'><img border='0' src='../images/smallearth.gif' height='10'></a>]]></cell>");	
				print("</row>");
				$posStart++;
			}
		}else{
