...
ini_set("memory_limit","24M");
  ...
  $ppnow=$pnow['year'] . "-" . $pnow['mon'] . "-" .$pnow['mday'];
    $sqlstr="SELECT alage.*, station.*, project.* FROM (alage INNER JOIN project ON alage.Project_id = project.Project_id) INNER JOIN station ON alage.id = station.id  where record_id=".$_GET['record_id'];
	...
	echo "<dwc:occurrenceID>urn:lsid:wlterm.biodiv.sinica.edu.tw:observation:AO" . $arr[$index]['record_id']."</dwc:occurrenceID>";
	echo "<dwc:catalogNumber>".$arr[$index]['record_id']."</dwc:catalogNumber>";
         ...
         echo "<dwc:occurrenceDetails>全天光空域:" .  $arr[$index]['全天光空域'] . ", 直射光空域:" . $arr[$index]['直射光空域'] . ", 測站描述:" . $arr[$index]['Locality_Describe']."</dwc:occurrenceDetails>";
		  echo "<item>".$arr[$index]['item']."</item>"; 
         echo "<itemInChinese>".$arr[$index]['item_chinese']."</itemInChinese>"; 
         echo "<bioMass>".$arr[$index]['biomass']."</bioMass>"; 
         echo "<unit>".$arr[$index]['unit']."</unit>"; 
